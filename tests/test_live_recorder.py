...
def _make_fake_service(
    ...
    The recorder calls execute_script in three forms:
      • install observer (returns {installed, ...})
      • poll observer    (returns {installed, rev, matched, url, ready})
      • body innerText fallback (returns a string)

    We dispatch by inspecting the script source.
    """
    service = MagicMock()
    state = {"i": 0, "lock": Lock()}
    install_returns = {"installed": True, "reused": False, "matched": 1}

    def execute_script(script: str, *args: object) -> object:
        with state["lock"]:
            i = state["i"]
            if "MutationObserver" in script:
                return install_returns
            if "ready: document.readyState" in script or "ready" in script and "rev" in script:
                # Polling — advance the cursor
...
def test_recorder_captures_mutation_observer_revisions_at_same_url(
    ...
    """The field bug: URL never changes, content swaps via AJAX.

    The MutationObserver bumps `rev` each time the watched area's text changes;
    the recorder must capture once per revision.
    """
    same_url = "https://www.actual4test.com/exam/C1000-185-questions"
    poll_states = [
        {"installed": True, "rev": 1, "matched": 5, "url": same_url, "ready": "complete"},
        {"installed": True, "rev": 1, "matched": 5, "url": same_url, "ready": "complete"},
        {"installed": True, "rev": 2, "matched": 5, "url": same_url, "ready": "complete"},
        {"installed": True, "rev": 2, "matched": 5, "url": same_url, "ready": "complete"},
        {"installed": True, "rev": 3, "matched": 5, "url": same_url, "ready": "complete"},
        {"installed": True, "rev": 3, "matched": 5, "url": same_url, "ready": "complete"},
    ]
    ...
    revisions = [c.revision for c in recorder.captures]
    assert revisions == [1, 2, 3]
    # All three live in distinct folders even though the URL is identical.
    ...
    assert [m["revision"] for m in manifest] == [1, 2, 3]
